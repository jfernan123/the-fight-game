...
FACING = {"red": 1.0, "blue": -1.0}
START_X = {"red": -2.0, "blue": 2.0}
# Real human range-of-motion limits (degrees): (flex_max, extend_max) per joint, shared by the
# hard joint ranges and the explore-mode actuator sliders so both agree on the same limits.
ROM = {
    "shoulder": (170.0, 50.0),
    "elbow": (145.0, 0.0),
    "hip": (120.0, 25.0),
    "knee": (145.0, 0.0),
    # Trunk flexion (bend forward) vs extension (lean back) -- forward is much more mobile than
    # backward for a real spine, same asymmetry pattern as the limb joints above.
    "waist_bend": (60.0, 30.0),
}
# Sign of each joint's angle relative to `facing`, for its hard range AND its explore-mode
# slider. The knee is flipped (-1) because it bends opposite to the hip (see _rest_pose_deg).
RANGE_SIGN = {"shoulder": 1.0, "elbow": 1.0, "hip": 1.0, "knee": -1.0}
# Torso twist (yaw about the vertical axis) is symmetric left/right, unlike the flexion-biased
# limb joints above, so it doesn't need facing-mirroring -- just one +/- range around facing 0.
# Two separate DOFs, matching real hip-shoulder separation: the pelvis (legs attached) rotates
# relative to the ground, and the chest (arms/head attached) rotates relative to the pelvis
# through the spine -- they are NOT the same body, so one can move without dragging the other.
HIP_TWIST_MAX = 60.0
WAIST_TWIST_MAX = 45.0


def _rest_pose_deg(facing: float) -> dict[str, float]:
    """Guard-stance rest angles (degrees) that the joint springs hold each limb toward.

    The shoulder/elbow (and hip/knee) hinges share the same rotation axis, so a child joint's
    angle adds directly onto its parent's angle to get its absolute world orientation.

    The elbow must carry the *same* sign as the shoulder (matching `-lean`) so it keeps curling
    the same way the shoulder is already leaning -- like a bicep curl, which really does continue
    rotating in that same direction to bring the fist up in front of the body.

    The knee is the opposite: it must carry the *opposite* sign from the hip. A bent knee folds
    the shin *back* relative to the thigh so the foot stays under the body (sit in a chair: the
    thigh rotates ~90 degrees forward, but the shin hangs straight down again, i.e. its angle
    relative to the world is back near 0 -- that only works if hip and knee angles have opposite
    signs and partially cancel). Same-sign, like the elbow, would swing the whole leg in one
    continuous forward arc instead of a natural zigzag -- which is exactly what "inverted knees"
    looks like (confirmed via forward kinematics: the foot ended up 0.26 forward of the torso,
    well past the hip, instead of settling back under it).
    """
    lean = 25.0 * facing
...
def _signed_range(facing: float, flex_max: float, extend_max: float) -> str:
    """MJCF joint range mirrored by facing direction.

    'Flexion' (forward/up, e.g. raising an arm or bending a knee forward) and 'extension'
    (backward, e.g. reaching an arm behind the back) map to opposite angle signs depending on
    which way the fighter faces. `flex_max`/`extend_max` are real human range-of-motion limits
    in degrees, so a joint physically can't rotate past what a human body allows.
    """
    if facing > 0:
...
def _humanoid_body(name: str, body_rgba: str, accent_rgba: str, glove_rgba: str, free_torso: bool = False) -> str:
    """Build the MJCF for one humanoid fighter: torso, head, two legs, two arms, boxing gloves.

    `free_torso` adds two more hinge joints (pitch, roll) to the torso stack, on top of the
    always-present x/y/z slides and hip_twist yaw -- giving the torso real tip-over physics
    instead of being mechanically locked upright. Off by default (used only by the walker envs,
    walk_gym.py/walk_gym_mjx.py) so the fighter combat model is unchanged -- it deliberately keeps
    fighters from falling from punches, a separate design decision from the walker's balance task.
    Unlike hip_twist, these have no spring (stiffness 0) -- a real fall isn't restored to upright
    by a passive spring, the walking policy has to actively counteract gravity/momentum itself,
    which is the actual point of adding them.
    """
    facing = FACING[name]
    rest = _rest_pose_deg(facing)
    foot_x = 0.14 * facing

    def arm(side: str, y: float) -> str:
        return f"""
      <body name="{name}_upperarm_{side}" pos="0 {y} 0.22">
        <joint name="{name}_shoulder_{side}" type="hinge" axis="0 1 0" range="{_signed_range(facing * RANGE_SIGN['shoulder'], *ROM['shoulder'])}"
               stiffness="150" springref="{rest[f'shoulder_{side}']}" damping="15" armature="0.06" />
        <geom type="capsule" fromto="0 0 0 0 0 -0.24" size="0.06" mass="3.5" rgba="{body_rgba}" />
        <body name="{name}_forearm_{side}" pos="0 0 -0.24">
          <joint name="{name}_elbow_{side}" type="hinge" axis="0 1 0" range="{_signed_range(facing * RANGE_SIGN['elbow'], *ROM['elbow'])}"
                 stiffness="150" springref="{rest[f'elbow_{side}']}" damping="15" armature="0.06" />
          <geom type="capsule" fromto="0 0 0 0 0 -0.22" size="0.05" mass="2.4" rgba="{body_rgba}" />
          <geom name="{name}_glove_{side}" type="sphere" pos="0 0 -0.26" size="0.09" mass="1.3" rgba="{glove_rgba}" />
        </body>
      </body>"""

    def leg(side: str, y: float) -> str:
        return f"""
      <body name="{name}_thigh_{side}" pos="0 {y} -0.22">
        <joint name="{name}_hip_{side}" type="hinge" axis="0 1 0" range="{_signed_range(facing * RANGE_SIGN['hip'], *ROM['hip'])}"
               stiffness="800" springref="{rest[f'hip_{side}']}" damping="180" armature="0.1" />
        <geom type="capsule" fromto="0 0 0 0 0 -0.32" size="0.075" mass="6.9" rgba="{body_rgba}" />
        <body name="{name}_shin_{side}" pos="0 0 -0.32">
          <joint name="{name}_knee_{side}" type="hinge" axis="0 1 0" range="{_signed_range(facing * RANGE_SIGN['knee'], *ROM['knee'])}"
                 stiffness="800" springref="{rest[f'knee_{side}']}" damping="180" armature="0.1" />
          <geom type="capsule" fromto="0 0 0 0 0 -0.32" size="0.06" mass="5.2" rgba="{body_rgba}" />
          <geom name="{name}_foot_{side}" type="capsule" fromto="0 0 -0.32 {foot_x} 0 -0.36"
                size="0.05" mass="1.7" friction="1.5 0.005 0.0001" rgba="{body_rgba}" />
        </body>
      </body>"""

    return f"""
    <body name="{name}" pos="{START_X[name]} 0 {STAND_HEIGHT}">
      <joint name="{name}_x" type="slide" axis="1 0 0" damping="5" />
      <joint name="{name}_y" type="slide" axis="0 1 0" damping="5" />
      <joint name="{name}_z" type="slide" axis="0 0 1" />
      <joint name="{name}_hip_twist" type="hinge" axis="0 0 1" range="-{HIP_TWIST_MAX} {HIP_TWIST_MAX}"
             stiffness="300" springref="0" damping="30" armature="0.1" />
      {f'<joint name="{name}_pitch" type="hinge" axis="0 1 0" range="-80 80" damping="8" armature="0.05" />' if free_torso else ''}
      {f'<joint name="{name}_roll" type="hinge" axis="1 0 0" range="-80 80" damping="8" armature="0.05" />' if free_torso else ''}
      <geom name="{name}_pelvis" type="capsule" fromto="0 -0.12 -0.22 0 0.12 -0.22" size="0.09" mass="6.5" rgba="{body_rgba}" />
      <geom name="{name}_spine_low" type="capsule" fromto="0 0 -0.22 0 0 -0.03" size="0.085" mass="3.2" rgba="{body_rgba}" />
      <body name="{name}_chest" pos="0 0 -0.03">
        <joint name="{name}_waist_twist" type="hinge" axis="0 0 1" range="-{WAIST_TWIST_MAX} {WAIST_TWIST_MAX}"
               stiffness="200" springref="0" damping="20" armature="0.08" />
        <joint name="{name}_waist_bend" type="hinge" axis="0 1 0" range="{_signed_range(-facing, *ROM['waist_bend'])}"
               stiffness="250" springref="0" damping="25" armature="0.08" />
        <geom name="{name}_waist" type="capsule" fromto="0 -0.09 0 0 0.09 0" size="0.075" mass="3.2" rgba="{body_rgba}" />
        <geom name="{name}_spine_up" type="capsule" fromto="0 0 0 0 0 0.22" size="0.1" mass="4.3" rgba="{body_rgba}" />
        <geom name="{name}_chest_geom" type="capsule" fromto="0 -0.17 0.22 0 0.17 0.22" size="0.09" mass="5.4" rgba="{body_rgba}" />
        <geom name="{name}_neck" type="capsule" fromto="0 0 0.22 0 0 0.31" size="0.05" mass="0.9" rgba="{body_rgba}" />
        <geom name="{name}_head" type="sphere" pos="0 0 0.43" size="0.115" mass="4.8" rgba="{accent_rgba}" />
        {arm("r", 0.19)}
        {arm("l", -0.19)}
      </body>
      {leg("r", 0.11)}
...
def _explore_actuators(name: str) -> str:
...
def _build_model_xml(actuators_fn, fighters: tuple[str, ...] = ("red", "blue"), free_torso: bool = False) -> str:
    bodies = "\n".join(_humanoid_body(name, *FIGHTER_STYLE[name], free_torso=free_torso) for name in fighters)
    actuators = "\n".join(actuators_fn(name) for name in fighters)
    return f"""
